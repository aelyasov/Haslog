...
toDaikonLog_  daikonOptions  events 
    = 
    case entries of
      (_:_) -> ( [appTopPpt,initStatePpt] ++ appEventsPpts, records )   -- need at least two entries
      _       -> ([],[])  -- else produce empty daikon-log
    where
...
convertAppEventTuple :: (Value_, AppEvent_, Value_, LowLevelInfo)
...
extractDaikonVarDecls daikonOptions values 
   =
   sortBy (compare `on` (\(vn,_,_) -> intercalate "." $ reverse vn))  
   . removeDuplicates 
   . removeDynamicVars
   . (let
   ...
   . map (\(n,ty,oty,val) -> (n,oty,ty))
   . removeEmptyMultiples
   $ 
   values
   where
   
   isEmptyMultiple (Multiple []) = True
   isEmptyMultiple _             = False
   
   -- removing entries with empty Multiple; since these do not give any type-info.
   -- the exeception is if a variable always have an empty-Multiple value.
   removeEmptyMultiples [] = []
   removeEmptyMultiples ((n,ty,oty,Single v)    : therest) = (n,ty,oty,Single v) : removeEmptyMultiples therest
   removeEmptyMultiples ((n,ty,oty,Multiple []) : therest) = 
      case find (\(n',ty,oty,v) -> (n' == n) && not(isEmptyMultiple v)) therest of
         Nothing -> {- n is always empty-Multiple -} (n,ty,oty,Multiple []) : removeEmptyMultiples [ (n',ty,oty,v) | (n',ty,oty,v) <- therest, n' /= n]
         _       -> removeEmptyMultiples [ (n',ty,oty,v) | (n',ty,oty,v) <- therest, n' /= n || not(isEmptyMultiple v)]
         
   removeEmptyMultiples ((n,ty,oty,Multiple z)  : therest) = (n,ty,oty,Multiple z) : removeEmptyMultiples therest2
      where
      therest2 = [ (n',ty,oty,v) | (n',ty,oty,v) <- therest, n' /= n || not(isEmptyMultiple v)]
...
convertArray_  xREFmap varpath a
   ...
   theArray = case nub elemsTypes of
     ...
     -- HACK
     -- a corner case is if the array is empty; we will also get an empty 
     -- elemsTypes. But then we don't know what's the type. For now, this is solved
     -- by just defaulting to int array, and later we ignore the type information
     -- when collecting var-decls; this does NOT work if the array remains empty.
     -- To be fixed:
     []          ->  [(varpath_, DaikonType_ (Array_ Int_), "int[]", Multiple [])]
     _           ->  []
...
convertCollection_  xREFmap varpath c
   ...
   theCol = case nub elemsTypes of
     ...
     -- same HACK as above
     []          ->  [(varpath_, DaikonType_ (Array_ Int_), "Collection int", Multiple [])]
     _           ->  [] 
...
convertDictionary_  xREFmap varpath d
   ...
   theKeys = case nub keysTypes of
     ...
     -- same HACK as above
     []     ->  [(varpath1, DaikonType_ (Array_ Int_), "DictionaryKey int", Multiple [])]
     _           ->  []   
   ...
   theVals = case nub valsTypes of
     ...
     -- same HACK as above
     []     ->  [(varpath2, DaikonType_ (Array_ Int_), "DictionaryVal int", Multiple [])]
     _           ->  []      
