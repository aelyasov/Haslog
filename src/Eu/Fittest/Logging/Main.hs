...
import Eu.Fittest.Logging.XML.XML2KLFA
import Eu.Fittest.Logging.XML.XML2Synoptic
import Eu.Fittest.Logging.Synoptic.SynopticInvariantChecker
...
main_ args = case getOpt Permute options args of
   ...
   interpret o fn outputFile
     ...
     | CmdToRaw `elem` o      =  xml2raw fn outputFile        -- converting fn back to raw log
     ...
     | CmdToKLFA `elem` o     = xml2klfa fn outputFile -- converting fn to klfa file (.csv)
     | CmdToSynoptic `elem` o = xml2synoptic fn outputFile -- converting fn to synoptic file (.csv)
     | CmdCheckSynopticInvs `elem` o = checkSynopticInvs fn outputFile
...
data Flag 
     ...
     | CmdToKLFA       -- convert FITTEST-xml log to the input format of the KLFA tool (lines of events with parameters)
     | CmdToSynoptic   -- convert FITTEST-xml log to the input foramt of the Synoptic tool (lines of event names)
     | CmdCheckSynopticInvs -- check Synoptic temporal invariants against new logs
...
optionTrees = [
   ...
   klfaOptions = opt1 "k" ["klfa"] (NoArg CmdToKLFA) "Produce an KLFA version of the given xml log file (.xml)."

   synopticOptions = opt1 "s" ["synoptic"] (NoArg CmdToSynoptic) "Produce a Synoptic version of the given xml log file (.xml)."
   checkSynopticInvs = opt1 "" ["check-synoptic-invs"] (NoArg CmdCheckSynopticInvs) "Check Synoptic temoral invariants against new logs"
