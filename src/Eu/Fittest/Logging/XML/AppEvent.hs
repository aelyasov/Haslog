...
getAppEventName e = if isNull name_ || isUndefined name_ || (getValType name_ /= "String") then Nothing
                    else Just (stripQuotes (vVal name_))  -- no func. composition in favor for speed!
   where
   e_ = aeEvent e
...
getAppEventTargetID e = if isNull id_ || isUndefined id_ 
                           || (getValType id_ /= "String")
                        then Nothing
                        else Just (stripQuotes (vVal id_))  -- no func. composition in favor for speed!
   where
   e_ = aeEvent e
   xrefmap  = buildXREFmap e_
   id_ = getField__ xrefmap "targetID" e_
   
-- to strip out the sorounding quotes of a string
...
testLowLevelParser1  parser log = uncurry (topRunCLparser parser) log    
...
exAppEvlog01  = testLowLevelParser1  (all_ parseAppEvent)  exAppEvlog0__
...
exLog2 = do 
  log <- readFile "../test/flx5_log_1382695459006_new.log"
  let -- plog = strParseRawLog log
      exAppEvlog0__ = internalRepRawLog__ log
      exAppEvlog01  = testLowLevelParser1  (all_ parseAppEvent)  exAppEvlog0__
  return exAppEvlog01    
